...
from bokeh.models import MultiLine,Quad,HoverTool,ResetTool,BoxZoomTool,PanTool,Range1d,Text,TapTool#,Line
...
import sys,pickle,ete3,collections
...
stuffcode="""
    ...
    for (var i = 0; i < inds.length; i++) {
        ...
        dtdata['accs'].push(data['gbacc'][inds[i]]);
    } 
    cds.change.emit();
    dtcds.change.emit();
"""
...
dtbl=DataTable(source=dtcds,columns=dtcolumns,width=200,height=100,editable=True,selectable=True)
...
layout=column(row(dtbl,column(pms,cms),column(gms,acw)),row(p1,p2))
