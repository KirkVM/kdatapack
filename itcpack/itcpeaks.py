...
def make_guided_figure(injpeak,injnum=1):
    fsx=LinearScale()
    fsy=LinearScale()
    xax=Axis(label='X',scale=fsx)
    yax=Axis(label='Y',scale=fsy,orientation='vertical')
    ...
    fig=Figure(marks=[flpr,blpts,*bls],axes=[xax,yax])
    fig.title=f'Injection {injnum}'
    blpts.enable_move=True
    blpts.restrict_y=True
    blpts.on_drag_end(injpeak.guided_callback)
    return fig
...
class ITCInjectionPeak:
    ...
    def create_guided_bl(self):
        ...
        for curidx in range(1,len(self.guided_bp_indices)):
            ...
            segseconds=self.seconds[self.guided_bp_indices[curidx-1]:self.guided_bp_indices[curidx]]
...
#        self.raw_heat=1e-6*xs_power_forint*((self.int_stop-self.int_start)/(stop_index-start_index))
    ...
    def guided_callback(self,name,value):
        self.dumb=name
        xval=value['point']['x']
        yval=value['point']['y']
